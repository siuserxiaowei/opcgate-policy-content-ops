...
在线版采用“双层闭环”：确定性证据引擎先限定可用来源、时效与风险；Cloudflare Workers AI 只在这些证据内改写草稿，模型失败时自动降级到本地确定性模板。
...
该脚本覆盖 1440×1000 桌面与 390×844 手机视口、输入模式切换、样例/手动分析、发布门禁、隐藏状态、横向溢出以及浏览器控制台错误。
...
- `assessPublicationRisk()`：识别保证获批、官方背书、金额、紧迫性等高风险措辞；
- `generateWeiboDraft()`：生成带“非实时样例”与人工核验提示的草稿；
- `buildVerificationChecklist()`：要求回到原始来源、检查隐私、核对政策并最终人工发布。
- `worker.js`：输入约束、Cloudflare Workers AI 受约束改写、失败降级和静态资源服务。
- `public/`：响应式“可信热点编辑台”，支持样例/手动输入、证据卡片、草稿编辑与核验清单。
...
- 草稿只供复制前审阅，程序没有微博登录、投稿或消息发送能力。
- 用户输入不写入数据库、不用于训练；线上请求仅在一次执行期间处理。
